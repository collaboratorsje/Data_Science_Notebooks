# -*- coding: utf-8 -*-
"""Lab - Ch6- Linear Model Regularization.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1eA5DRonJTQkUe_JRHlqFygnPOJV_LuJf
...
from sklearn.pipeline import Pipeline # to put model training process in a single step
from sklearn.decomposition import PCA # to help with PCA
from sklearn.cross_decomposition import PLSRegression # PLS
from ISLP.models import \
     (Stepwise,
      sklearn_selected,
      sklearn_selection_path) # These imports will help with subset selection techniques
!pip install l0bnb
...
Hitters = load_data('Hitters') # load data
np.isnan(Hitters['Salary']).sum() # print any null rows
...
Hitters = Hitters.dropna(); # remove the null rows that has NaN in salary
Hitters.shape # prints the new shape of the data after running Hitters.dropna()
...
We need a method that stores the full path of models found in forward selection, and allows predictions for each of these. This can be done with the `sklearn_selection_path()`
...
# define the forward stepwise strategy by passing the design without the intercept and the len(design.terms) or 19 and forward stepwise
...
full_path.fit(Hitters, Y) # train the different models according to the forward selection path
Yhat_in = full_path.predict(Hitters) # trest the different models
Yhat_in.shape # also includes the null model (so 20 different models)
...
# maybe the best model is step 6
...
# This step takes some time because of all the K folds
K = 5
kfold = skm.KFold(K,
                  random_state=0,
                  shuffle=True)
...
"""Based on the results from the cross-validation, the best model is model 6
...
D = design.fit_transform(Hitters) # fit and transform the dataset
D = D.drop('intercept', axis=1) # drop the intercept column
X = np.asarray(D) # convert the data into arrays
...
path = fit_path(X,
                Y,
                max_nonzeros=X.shape[1]) # using X.shape[1] to index the value
...
Xs = X - X.mean(0)[None,:] # compute the mean
X_scale = X.std(0) # compute the standard deviation
Xs = Xs / X_scale[None,:] # scale or normalize or standardized

lambdas = 10**np.linspace(8, -2, 100) / Y.std() # create set of lambdas

soln_array = skl.ElasticNet.path(Xs, # provide standardized data
                                 Y,
                                 l1_ratio=0.,
                                 alphas=lambdas)[1]
soln_array.shape
...
"""In ridge you cannot have values that get all the way to zero, and the lower the log the higher the value
...
"""Ridge regression gives a more stable model
...
pca = PCA(n_components=3) # create PCA compoenents to have two PCs
linreg = skl.LinearRegression() # create the model
pipe = Pipeline([('pca', pca),
                 ('linreg', linreg)])
pipe.fit(X, Y)
pipe.named_steps['linreg'].coef_

"""The negative values are hard to interpret
...
pls = PLSRegression(n_components=2,
                    scale=True) # PLS is different from PCA because we use labels
pls.fit(X, Y)
...
"""## let me know which technique to choose
## PCA vs PLS
PLS because it uses the Y and can be used for a predictive model, though it might be more computationally intensive.
